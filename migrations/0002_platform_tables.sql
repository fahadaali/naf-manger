-- جداول المنصة — كلُّ ما تقرؤه الشاشات وتكتبه، ما عدا `members` في `0001`.
--
-- إنشاءُ مخطّطٍ لا ترحيلُ بيانات: القاعدة تبدأ فارغة، والصفوف تُدخَل من
-- الشاشات.
--
...
--    لا يُسقط الطلب.
--
-- والأعمدة التي تحمل معرّف عضو — `assigned_to` و`user_id` و`created_by` و
-- `updated_by` — تشير إلى `members(user_id)` منطقياً لا بقيدٍ أجنبي: صفُّ
-- العضو يُنشأ بأول دخولٍ له، وقيدٌ هنا يُسقط إدراجاً باسم عضوٍ لم يدخل بعد.
